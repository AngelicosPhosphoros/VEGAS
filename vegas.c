      ...
      do
        {
          ...
          intgrl += mean * calls_per_box;

          f_sq_sum = variance * calls_per_box;
     ...
     if (wgt > 0.0)
       {
         ...
         double mean = (sum_wgts > 0) ? (wtd_int_sum / sum_wgts) : 0;
         double variance = intgrl - mean;
...
/* The new formula below computes exactly the same quantity as above
   but using a stable recurrence */
         if (state->samples == 1) {
           ...
           state->chisq += (wgt / (1 + (wgt / sum_wgts))) * variance * variance;
